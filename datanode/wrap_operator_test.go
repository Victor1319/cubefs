...
func newExtentStoreForOperatorTest(t *testing.T) (store *storage.ExtentStore) {
	...
	store, err = storage.NewExtentStore(path, 0, 1*util.GB, proto.PartitionTypeNormal, true, 0)
	require.NoError(t, err)
	return
}
...
func newDiskForOperatorTest(t *testing.T, dn *DataNode) (d *Disk) {
	...
	d.limitRead = newIOLimiter(1*util.MB, 10, 0)
	d.limitWrite = newIOLimiter(1*util.MB, 10, 0)
	return
}
